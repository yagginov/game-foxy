...
void MainMenu::_bind_methods() 
{
    ...
    ClassDB::bind_method(D_METHOD("_on_load_button_pressed"), &MainMenu::_on_load_button_pressed);
    ...
    ClassDB::bind_method(D_METHOD("get_load_button"), &MainMenu::get_load_button);
	ClassDB::bind_method(D_METHOD("set_load_button", "p_load_button_path"), &MainMenu::set_load_button);
	ClassDB::add_property("MainMenu", PropertyInfo(Variant::NODE_PATH, "load_path"), "set_load_button", "get_load_button"); 
...
void MainMenu::_ready() 
{
    ...
    if (has_node(load_path)) 
    {
        load = get_node<Button>(load_path);
    }
    if (load)
    {
        load->connect("pressed", Callable(this, "_on_load_button_pressed"));
    }
...
void MainMenu::_on_load_button_pressed()
{
    if (!load)
    {
        return;
    }
    if (has_node("LoadMenu"))
    {
        LoadMenu* load_menu = get_node<LoadMenu>("LoadMenu");
        load_menu->show();
        load_menu->initialize_load();
    }
}
...
void MainMenu::set_load_button(const NodePath& p_load_button_path)
{
    load_path = p_load_button_path;
}
NodePath MainMenu::get_load_button() const
{
    return load_path;
}
